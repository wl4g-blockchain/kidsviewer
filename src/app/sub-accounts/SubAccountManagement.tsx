...
export const SubAccountManagement: React.FC = () => {
  ...
  const fetchTenantInfo = async () => {
    try {
      const response = await fetch('/api/v1/sys/tenant/info');
      if (response.ok) {
  ...
  const fetchSubAccounts = async () => {
    try {
      const response = await fetch('/api/v1/sys/tenant/sub-accounts');
      if (response.ok) {
        const data = await response.json();
        setSubAccounts(data);
      }
  ...
  const handleDeleteSubAccount = async (accountId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/sys/tenant/sub-accounts?id=${accountId}`, {
        method: 'DELETE',
      });
